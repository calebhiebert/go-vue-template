...
var (
	jobDBTypes = map[string]string{`ID`: `uuid`, `Type`: `character varying`, `Priority`: `integer`, `Source`: `character varying`, `Data`: `jsonb`, `RunAt`: `timestamp without time zone`, `FailureData`: `jsonb`, `MaxRetries`: `integer`, `RetryCount`: `integer`, `Status`: `character varying`, `FinishedAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`}
	_          = bytes.MinRead
)
