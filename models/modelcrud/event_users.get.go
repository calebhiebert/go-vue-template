...
type GetEventUsersResponse struct {
	EventUsers models.EventUserSlice `json:"event_users"`
	Total      int64                 `json:"total"`
	NextOffset int64                 `json:"next_offset"`
}

type APIGetEventUsersResponse struct {
	EventUsers []APIEventUser `json:"event_users"`
	Total      int64          `json:"total"`
	NextOffset int64          `json:"next_offset"`
}

// GetEventUserByID godoc
// @Summary Gets a single EventUser entity by their id
// @Produce json
// @Success 200 {object} APIGetEventUsersResponse
// @Param id path string true "EventUser id"
// @Router /crud/eventUsers/:id [get]
func (*GeneratedCrudController) GetEventUserByID(c *gin.Context) {
	id := c.Param("id")
	...
	EventUser, err := models.EventUsers(qm.Where("id = ?", id)).OneG(c.Request.Context())
	if err != nil {
		api.APIErrorFromErr(err).Respond(c)
		return
	}

	c.JSON(http.StatusOK, EventUser)
}
...
// @Param withDeleted query string false "Include deleted eventUsers in the results"
...
// @Param sort.is_owner query string false "Sort by is_owner. Value should be ASC or DESC. eg: ?sort.created_at=DESC"
// @Param sort.accepted query string false "Sort by accepted. Value should be ASC or DESC. eg: ?sort.created_at=DESC"
...
// @Router /crud/eventUsers [get]
func (*GeneratedCrudController) GetEventUsers(c *gin.Context) {
	...
	for q, v := range c.Request.URL.Query() {
		...
		case "sort.is_owner":
			orderBy = append(orderBy, "is_owner "+sortDirection)
		case "is_owner.eq":
			queryMods = append(queryMods, qm.Where("is_owner = ?", v[0]))
		case "is_owner.null":
			if v[0] == "true" {
				queryMods = append(queryMods, qm.Where("is_owner IS NULL"))
			} else {
				queryMods = append(queryMods, qm.Where("is_owner IS NOT NULL"))
			}

		case "sort.accepted":
			orderBy = append(orderBy, "accepted "+sortDirection)
		case "accepted.eq":
			queryMods = append(queryMods, qm.Where("accepted = ?", v[0]))
		case "accepted.null":
			if v[0] == "true" {
				queryMods = append(queryMods, qm.Where("accepted IS NULL"))
			} else {
				queryMods = append(queryMods, qm.Where("accepted IS NOT NULL"))
			}
	...
	count, err := models.EventUsers(queryMods...).CountG(c.Request.Context())
	if err != nil {
		api.APIErrorFromErr(err).Respond(c)
		return
	}
	...
	eventUsers, err := models.EventUsers(queryMods...).AllG(c.Request.Context())
	if err != nil {
		api.APIErrorFromErr(err).Respond(c)
		return
	}
	...
	if eventUsers == nil {
		eventUsers = models.EventUserSlice{}
	}

	c.JSON(http.StatusOK, GetEventUsersResponse{
